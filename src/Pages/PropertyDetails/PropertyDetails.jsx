...
import MapDescriptionModal from "./components/MapDescriptionModal";
...
const PropertyDetails = () => {
  ...
  const { checkInDate, checkOutDate, showMobileReserveModal } = useSelector(
    (state) => state.ComponentState
  );
  const {
    reservation: { summary_details, price },
  } = useSelector((state) => state.reservationState);
  ...
  const CleaningFee =
    reserve === "succeeded" && summary_details[0]?.total_cleaning_price
      ? parseInt(summary_details[0]?.total_cleaning_price)
      : 0;
  const PickupFee =
    reserve === "succeeded" && summary_details[0]?.total_pickup_dropoff_price
      ? parseInt(summary_details[0]?.total_pickup_dropoff_price)
      : 0;
  const carPrice =
    reserve === "succeeded" && summary_details[0]?.total_car_price
      ? parseInt(summary_details[0]?.total_car_price)
      : 0;
  const driverPrice =
    reserve === "succeeded" && summary_details[0]?.total_driver_price
      ? parseInt(summary_details[0]?.total_driver_price)
      : 0;
  ...
  const addDays = () => {
    const days =
      summary_details[0]?.stay_length && summary_details[0]?.stay_length;
  ...
  useEffect(() => {
    ...
    navigate(
      `/apartment/${Id}&checkIn=${
        checkInDate !== null ? checkInDate : ""
      }&checkOut=${checkOutDate !== null ? checkOutDate : ""}`
    );
  }, [
    dispatch,
    checkInDate,
  ...
  if (status === "failed") {
    return <Error title="Property not found" Icon={SearchNotFoundIcon} />;
  }

  return (
    <>
      {openService && (
        <Backdrop onClick={() => setOpenService(false)} zIndex="10" />
      )}
      {showModal && (
        <Backdrop
        ...
        <Main paddingleft="true" paddingRight="true">
          <Header>
            {status === "loading" ? (
              ...
              status === "succeeded" && <PropertyName status={status} />
            )}
            ...
            <BodyContainer>
              <BodyContent>
                {status === "loading" ? (
                  ...
                  status === "succeeded" && <PropertyHeader status={status} />
                )}
                {status === "loading" ? (
                  <Description>
          ...
          <BodyContainer style={{ margin: "max(3vw,2rem) 0" }}>
            <BodyContent>
              <PropertyCalender
                status={reserve}
              ...
              />
              {/* //! Push to demo: Commit it to test branch */}
