...
const SelectDateInput = () => {
    ...
    return (
        <>
            {show &&
                (
                <CalenderModal  show={show} setShow={setShow}  theme='' top="12vh" right={Query ? '250px' : '180px'}width="fit-content" calanders={1} disablebooked  initial={{opacity: 0, y: 0, x: '100%'}} animate={{opacity: 1, y: 70, x: '50%'}} />
                )
            }
            <Wrapper>
                <InputContainer onClick={() => setShow(!show)}>
                    <span>{reserve === 'loading' ? <SkeletonLoader /> : 'Check-in'}</span>
                    {reserve === 'loading' ? <SkeletonLoader /> : <input type="text" value={ checkInDate ? checkInDate : 'DD-MM-YY'} disabled /> }
                </InputContainer>
                <InputContainer  onClick={() => setShow(!show)}>
                    <span>{reserve === 'loading' ? <SkeletonLoader /> : 'Check-out'}</span>
                    {reserve === 'loading' ? <SkeletonLoader /> : <input type="text" value={checkOutDate ? checkOutDate : 'DD-MM-YY'} disabled /> }
                </InputContainer>
            </Wrapper>
        </>
    )
